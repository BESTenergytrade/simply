...
def load(dirpath, data_format, batch_end=None):
    """
    ...
    if data_format == "csv":
        ...
        for aj in actors_j["actors"].values():
            if aj["assignedMarket"] in existing_markets:
                ...
                # partial_data_read todo: limit a["df"] to first batch of timesteps + horizon
                # partial_data_read todo: dirpath / aj["csv"] in actor speichern
        ...
        for mj in actors_j["marketMakers"].values():
            ...
            if len(filtered_market) > 0:
                ...
                # partial_data_read todo: limit m_df to first batch of timesteps + horizon
                # partial_data_read todo: dirpath / mj["csv"] in MM speichern
