...
class SimplyPPO(PPO):
    ...
    :param env: The custom simply gym environment to learn from (if registered in Gym, can be str)
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            actions = self.env.envs[0].env.get_action()
            new_obs, rewards, dones, infos = env.step(actions)
            if isinstance(self.action_space, spaces.Discrete):
                # Convert discrete action from float to long
                actions = actions.long().flatten()

            # get values and log_prob from policy by evaluating action
            # reshape action and observation and convert to tensors
            actions_array = actions[None]
            with th.no_grad():
                # Convert to pytorch tensor or to TensorDict
                actions_tensor = th.as_tensor(actions_array)
                obs_tensor = obs_as_tensor(self._last_obs, self.device)
            # determine values and log_prob
            values, log_probs, entropy = self.policy.evaluate_actions(obs_tensor, actions_tensor)
            # convert values and log_prob to correct format for buffer logging
            values, log_probs = values.detach(), log_probs.detach()
            # print(values, log_probs)
